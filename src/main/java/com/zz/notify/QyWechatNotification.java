package com.zz.notify;

import com.alibaba.fastjson.JSON;
import com.arronlong.httpclientutil.exception.HttpProcessException;
import com.zz.notify.dto.BuildBeginInfo;
import com.zz.notify.dto.BuildGitInfo;
import com.zz.notify.dto.BuildMentionedInfo;
import com.zz.notify.dto.BuildOverInfo;
...
public class QyWechatNotification extends Publisher implements SimpleBuildStep {
    ...
    public boolean prebuild(AbstractBuild<?, ?> build, BuildListener listener) {
        ...
        BuildGitInfo buildGitInfo = getGitInfo(envVars);
        ...
        String req = buildInfo.toJSONString(buildGitInfo);
    ...
    public void perform(Run<?, ?> run, FilePath workspace, Launcher launcher, TaskListener listener) throws InterruptedException, IOException {
        ...
        listener.getLogger().println("运行结果：" + result);
    ...
    public BuildGitInfo getGitInfo(EnvVars envVars){
        BuildGitInfo buildGitInfo = new BuildGitInfo();
        buildGitInfo.setGitCommit(envVars.get("GIT_COMMIT"));
        buildGitInfo.setGitAuthorName(envVars.get("GIT_AUTHOR_NAME"));
        buildGitInfo.setGitAuthorEmail(envVars.get("GIT_AUTHOR_EMAIL"));
        buildGitInfo.setGitBranch(envVars.get("GIT_BRANCH"));
        buildGitInfo.setGitCommitterName(envVars.get("GIT_COMMITTER_NAME"));
        buildGitInfo.setGitCommitterEmail(envVars.get("GIT_COMMITTER_EMAIL"));
        buildGitInfo.setGitPreviousCommit(envVars.get("GIT_PREVIOUS_COMMIT"));
        buildGitInfo.setGitPreviousSuccessfulCommit(envVars.get("GIT_PREVIOUS_SUCCESSFUL_COMMIT"));
        buildGitInfo.setGitLocalBranch(envVars.get("GIT_LOCAL_BRANCH"));
        buildGitInfo.setGitUrl(envVars.get("GIT_URL"));
        buildGitInfo.setChanges(envVars.get("CHANGES_SINCE_LAST_SUCCESS"));
        return buildGitInfo;
    }
    ...
    public NotificationConfig getConfig(EnvVars envVars){
        ...
        System.out.println("78787878:"+JSON.toJSONString(config));
      ...
      if(StringUtils.isNotEmpty(customerText)){
          config.customerText = customerText;
      }
        ...
        if(config.customerText.contains("$")){
            String val = NotificationUtil.replaceMultipleEnvValue(config.customerText, envVars);
            config.customerText = val;
        }
        return config;
    }
