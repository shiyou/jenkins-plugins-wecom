...
public class BuildOverInfo {
    ...
    public String toJSONString(){
        ...
        if(StringUtils.isNotEmpty(this.customerText)) {
            content.append(" >代码扫描结果:  \n" ).append(this.customerText);
        }
    ...
    public String sonarResult(){
        String projectName = "";
        StringBuilder sonarResult = new StringBuilder(" > sonar 代码扫描结果分析情况：");
        sonarResult.append("当前bug数量：").append("比上一次部署新增bug数：");
        sonarResult.append("当前bug数量：").append("比上一次部署新增bug数：");

        sonarResult.append(" > 查看更多扫描结果：http://192.168.1.127:9000/dashboard?id=").append(projectName);
